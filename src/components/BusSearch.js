...
// Generate dynamic preset searches based on today's date
const generatePresetSearches = () => {
  ...
  return [
    { from: 'Chennai', to: 'Vellore', date: formatDate(today) },
    { from: 'Kanchipuram', to: 'Vellore', date: formatDate(today) },
    { from: 'Chennai', to: 'Pondicherry', date: formatDate(today) },
    { from: 'Chennai', to: 'Coimbatore', date: formatDate(today) },
    { from: 'Madurai', to: 'Chennai', date: formatDate(today) }
  ];
};

const PRESET_SEARCHES = generatePresetSearches();
...
const BusSearch = () => {
  ...
  useEffect(() => {
    const fetchAllBuses = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL || '/api'}/buses`);
  ...
  const fetchSearchResults = async (params) => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_BASE_URL || '/api'}/buses`, { params });
      const sortedBuses = res.data.sort((a, b) => new Date(a.departureTime) - new Date(b.departureTime));
      setBuses(sortedBuses);
      setRecommendations([]);
      setConnectingRoutes([]);
      ...
      if (sortedBuses.length === 0 && params.from && params.to) {
        const recRes = await axios.get(`${process.env.REACT_APP_API_BASE_URL || '/api'}/buses`);
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 1 }}>
          <Grid item xs={12} sm={4}>
            <Typography variant="subtitle2" sx={{ fontWeight: 600, color: '#424242', fontSize: '0.9rem' }}>
              From
            </Typography>
          </Grid>
          <Grid item xs={12} sm={4}>
          ...
          <Grid item xs={12} sm={4}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
              <CalendarTodayIcon sx={{ color: 'primary.main', fontSize: '0.95rem' }} />
              <Typography variant="subtitle2" sx={{ fontWeight: 600, color: '#424242', fontSize: '0.9rem' }}>
                Departure Date
              </Typography>
            </Box>
        ...
        {/* Today's Date Helper */}
        <Typography variant="caption" sx={{ color: 'primary.main', fontWeight: 500, mt: 1.5, display: 'block', ml: 0 }}>
